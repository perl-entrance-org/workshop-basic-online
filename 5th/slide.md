...
サブルーチンを定義すると、自分が必要な関数を作成することができます。
...
Perl でサブルーチンを定義する為には、`sub サブルーチン名 { ... }` と書きます。
...
`return`はサブルーチンの中に複数個書くことができますが `return`に到達した場合、それ以降の処理は一切行われず、すぐさま値を返してサブルーチンの実行を終了します。（ガード節といいます）
