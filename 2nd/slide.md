...
- 1 から 9 までの好きな数字をスカラー変数に格納し、それらの値を表示するスクリプト `scalar_practice1.pl` を作成しよう！
...
- Perl 入学式の情報をスカラー変数に格納し、それらの値を表示するスクリプト `scalar_practice3.pl` を作成しよう！
...
1. 変数 `$num1` に 10, `$num2` に 3 を代入し、それぞれを画面に表示させる。

1. 変数 `$num1` から 変数 `$num2` を引いた数を画面に表示させる。

1. 変数 `$num1` を 変数 `$num2` で割った時の余りを表示させる。
...
以下のコードをコピペして、正しく出力される様に変数を連結するコード `strconcat.pl` を続けて書いてみましょう。
...
上のコードは、数値演算と文字列連結を混ぜた場合の「アンチパターン（良くない例）」です。
...
- この「入力」を用いて、スクリプトの実行の都度、中身や出力を変えることができます。
...
- 様々な入力のうち、キーボードから入力されるテキストデータのことを<ruby>**標準入力**<rt>ひょうじゅんにゅうりょく</rt></ruby>(Standard Input)といいます。
  ...
  - 標準ではない入力：マウスからの入力、ファイルからの入力、音声入力、カメラ、画面へのタッチ・・・
...
Wandbox で標準入力を受け取るためには、以下の手順となります。

1. エディタ部の下にある `Stdin` と書かれたところをクリックする

1. 入力欄に記入する。1 行につき、1 つの標準入力として入力される

<img src="https://raw.githubusercontent.com/perl-entrance-org/workshop-basic-online/master/2nd/image/wandbox09.png" align='left'>
...
- 名前の入力を求る "Your Name?> " と表示する

- 標準入力を用いて名前を入力する

- 入力された名前の左側に Hello、右側に "!" を付けて表示する
...
標準入力 `<STDIN>` から 0 以外の整数を 2 つ読み込み、 それらを四則演算(`+`, `-`, `*`, `/`)した結果を下の例のように表示するスクリプト `calc.pl`を作成しよう。
...
ヒント：以下のようにすることで複数回の標準入力を利用できます
...
Wandbox では Stdin の欄に「最初の数字」と「次の数字」を改行して入力しましょう。
...
他のプログラミング言語では、`true` や `false` という単語を 真 または 偽 の意味を持たせたキーワードとして扱うことがあります。
...
標準入力から数字を一つ入力し、その数字とスクリプト中の `$answer` とを比較して、次ページの条件に従った表示をするスクリプトを作ってください。
