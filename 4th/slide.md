...
1. 次に用意されたスカラー変数から、スカラー変数リファレンス `$language_ref` を作成しよう。

```perl
my $language = "Perl";
```

2. 次に用意された配列とハッシュから、配列リファレンス `$students_ref`、ハッシュリファレンス `$scores_ref` を作成しよう。
...
3. 1.2.で作成した3つのリファレンスを画面に出力してみよう。
